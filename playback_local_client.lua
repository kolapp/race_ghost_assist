-- // =====================[ important variables ] ===================//
-- TODO: EXPORT FUNCTIONS TO CHANGE SETTINGS FROM A GUI
-- *TODO: FOR LOOP NEEDED IN NEARBY SEARCH <-- k
-- *TODO: ROLL BACK A FEW NODES WHEN DYING <-- k
-- *TODO: LOAD LINES ON /ASSIST <-- nope, too complicated
...
function show()
    if not drawRacingLine_HANDLER then
        drawRacingLine_HANDLER = function()
            drawRacingLine()
        end
...
addEventHandler("onClientResourceStart", getRootElement(), function(startedRes)
    if startedRes == getThisResource() then
        ...
        -- scroll back a few nodes on death
        addEventHandler("onClientPlayerWasted", getLocalPlayer(), function()
            if Settings["enable"] == "on" then
                -- !!!
                nextNodeID = math.clamp(1, nextNodeID - 25, nextNodeID) -- ugly const here
                -- stop timer
                if isTimer(assistTimer) then
                    killTimer(assistTimer)
                    assistTimer = nil
                    -- outputChatBox("stop timer", 255, 0, 0)
                end
                -- debug
                -- outputChatBox("onClientPlayerWasted", 255, 0, 0)
            end -- if
        end -- func
        )

        -- resume the assistTimer on spawning again
        addEventHandler("onClientPlayerSpawn", getLocalPlayer(), function()
            if Settings["enable"] == "on" and recording then
                -- resume timer
                if not isTimer(assistTimer) then
                    assistTimer = setTimer(updateRacingLine, 150, 0)
                    outputChatBox("start timer", 255, 0, 0)
                end
                -- debug
                -- outputChatBox("onClientPlayerSpawn", 255, 0, 0)
            end -- if
        end -- func
        )
        ...
        -- TODO: THIS DELETES TOP GHOST AT THE WRONG TIME
    ...
    if ghost then
        ...
        while (node) do
            if type(node) ~= "userdata" then
                outputDebugString("race_ghost - playback_local_client.lua: Invalid node data while loading ghost: " ..
                                      type(node) .. ":" .. tostring(node), 1)
                break
            end
...
addEventHandler("onClientMapStarting", g_Root, function(mapInfo)
...
addEventHandler("onClientGhostDataReceive", g_Root, function(rec, bestTime, racer, _, _)
    ...
    if Settings["enable"] == "on" and Settings["mode"] == "top" then
        ...
        outputDebug("ghost loaded, starting assist") -- DEBUG
        outputChatBox(
            "[Racing Assist] #ffffffGhost by " .. RemoveHEXColorCode(racer) .. " @" .. msToTimeStr(bestTime) ..
                " loaded.", 255, 170, 64, true)
...
function updateRacingLine()
    ...
    while (recording[id]) do
        ...
        -- TODO: dst < 50 can snap to the wrong road sometimes
        if (dst < 200 and id >= lastNodeID) then -- ugly constant here
            nextNode = id
            break
        end
        id = id + 1
    ...
    if (nextNode ~= nil) then
        ...
        if (vehicle) then
            ...
            -- move it max 10 steps closer to player on every iteration, break if node is close enough
            for i = 1, 10 do
                prev = recording[nextNode + i - 1] -- id + 0..9
                ...
                if (curr and prev) then
                    ...
                    -- we can stop scrolling forward, the next node would be further than the prev
                    if (prev_dst > dst) then
                        -- !!!
                        -- this will be the nearest valid node to player
                        nextNodeID = idx
                        -- outputChatBox("i: "..i) -- DEBUG
                        -- !!!
                    else
                        break
                    end
...
function drawRacingLine()
    ...
    -- if (dst < 200) then
    -- -- one nearby node
    -- dxDrawLine3D (node1.x, node1.y, node1.z-0.6, node1.x, node1.y, node1.z-0.4, tocolor (255,0,0, 255), 25, true)
    -- -- store nearby points
    ...
    -- if (node1) then
    -- dxDrawLine3D (node1.x, node1.y, node1.z-0.6, node1.x, node1.y, node1.z-0.4, tocolor(0,255,0, 255), 40, true)
    -- end		
    ...
    -- -- -- DEBUG
    -- -- highlight last node
    -- -- dxDrawText("lastNodeID: "..lastNodeID, 800, 440, 1920, 1080, tocolor(255, 128, 0, 255), 1, "pricedown")
    -- -- if recording[lastNodeID] then
    -- -- dxDrawLine3D(recording[lastNodeID].x, recording[lastNodeID].y, recording[lastNodeID].z, recording[lastNodeID].x, recording[lastNodeID].y, recording[lastNodeID].z+20, tocolor(0, 255,0, 128), 8, true)
    -- -- end
    ...
    for i = start, start + Settings["linelength"], 1 do
        ...
        if (node1 and vehicle) then
            ...
            -- if i == start then dxDrawText ("my_dst: ".. my_dst, 800, 520, 1920, 1080, tocolor(255, 128, 0, 255), 1, "pricedown") end 						
            ...
            if not gx and abs(rx) < 80 and abs(ry) < 70 then
                -- getGroundPosition looks shit outside of these limits
            ...
            if gx then
                ...
                -- !!!
                if (gx and xx2 and -- current and prev node is valid
                i ~= start and -- dont connect 1st and last node
                my_dst < 100 -- dont draw when the car is too far
                ) then
                    dxDrawMaterialLine3D(gx, gy, gz, xx2, yy2, zz2, img, arrowSize,
                        tocolor(color_r, color_g, color_b, color_a), xx, yy, zz)
                end -- gx
